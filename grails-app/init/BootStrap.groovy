...
//            s2 = new Scholar ( firstName: "Killgur", lastName: "Alex", dateOfBirth: new Date().parse( "yyyy.MM.dd",'1981.04.09' ) ).save( failOnError: true )
        ...
        if ( !Teacher.count() ) {
            t1 = new Teacher ( firstName: "Max", lastName: "Mad", dateOfBirth: new Date().parse( "yyyy.MM.dd",'1981.04.09' ), lesson: l1, audience: a1, scholar: s2 ).save( failOnError: true )
            t2 = new Teacher ( firstName: "Mad", lastName: "Max", dateOfBirth: new Date().parse( "yyyy.MM.dd",'1981.04.09' ), lesson: l2, audience: a2, scholar: s1 ).save( failOnError: true )
//            t1 = new Teacher ( firstName: "Max", lastName: "Mad", dateOfBirth: new Date().parse( "yyyy.MM.dd",'1972.04.09' ) ).save( failOnError: true )
//            t2 = new Teacher ( firstName: "Mad", lastName: "Max", dateOfBirth: new Date().parse( "yyyy.MM.dd",'1972.04.09' ) ).save( failOnError: true )
        }

        if ( !Groups.count() ) {
            g1 = new Groups ( name: "KM-83", course: "Computer Science", scholar: s1, audience: a2 ).save( failOnError: true )
            g2 = new Groups ( name: "RM-84", course: "Computer Science", scholar: s2, audience: a1 ).save( failOnError: true )
//            g3 = new Groups ( name: "KM-83", course: "Computer Science", scholar: s2, audience: a2 ).save( failOnError: true )
        }
    }
